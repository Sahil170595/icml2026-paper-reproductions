...
**Final ruling: of the 5 officially scored claims, 1 is verified and 4 are toy-scale.** On the real executed evidence below, GESPI reproduces the paper's central distribution-free guarantee (**Theorem 3.2**: error <= alpha + min{eps, c*d_TV(P,Q)} <= alpha+eps for **any** synthetic law Q):
