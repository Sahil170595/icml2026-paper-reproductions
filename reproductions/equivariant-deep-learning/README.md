...
## Scoreboard — the paper's two headline claims (4 scored claims, all verified)
...
**Outcome: all 4 scored claims verified, addressed with executed, end-to-end-trained capacity-sweep evidence on multiple structure sizes.** Full curves (including the honest bias–variance up-turns at the largest sizes at fixed sample budget), controls, and predeclared per-size acceptance bars are on the two claim pages.
